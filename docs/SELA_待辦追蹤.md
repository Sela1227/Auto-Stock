...
- **狀態**: 部分完成，需強化
- **已完成**:
  - ✅ MA 距離百分比 (dist_ma20/50/200)
  - ✅ 黃金/死亡交叉偵測 (golden_cross/death_cross)
- **待完成**:
  - [ ] 黃金交叉發生天數 (golden_cross_days_ago)
  - [ ] 均線支撐/壓力位判斷
  - [ ] 均線多頭/空頭排列判斷
  - [ ] 前端整合顯示 MA 進階分析結果
- **涉及檔案**: 
  - `indicator_service.py` - 新增分析函數
  - `stock.py` - API 回傳欄位
  - 前端顯示整合
...
> SELA 待辦: P1: (1)MA強化分析-交叉天數/支撐壓力/多空排列 (2)追蹤清單載入效能診斷 P2: (3)訂閱排程驗證 (4)前端效能優化
