...
class TransactionCreate(BaseModel):
    ...
    name: Optional[str] = Field(None, max_length=100, description="è‚¡ç¥¨åç¨±")
    market: str = Field(..., pattern="^(tw|us)$", description="å¸‚å ´ (tw/us)")
    transaction_type: str = Field(..., pattern="^(buy|sell)$", description="äº¤æ˜“é¡žåž‹ (buy/sell)")
    quantity: int = Field(..., gt=0, description="è‚¡æ•¸ï¼ˆå°è‚¡ï¼šå¼µÃ—1000 + é›¶è‚¡ï¼‰")
    price: float = Field(..., gt=0, description="æˆäº¤åƒ¹")
...
class ExchangeRateUpdate(BaseModel):
    """æ›´æ–°åŒ¯çŽ‡è«‹æ±‚"""
    rate: float = Field(..., gt=0, description="USD/TWD åŒ¯çŽ‡")
...
async def import_transactions(
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"åŒ¯å…¥å®Œæˆï¼šæˆåŠŸ {len(added)} ç­†ï¼Œå¤±æ•— {len(errors)} ç­†",
            "data": {
                "added": added,
                "errors": errors,
...
@router.get("/exchange-rate", summary="å–å¾—åŒ¯çŽ‡")
async def get_rate(
    db: AsyncSession = Depends(get_async_session),
):
    """å–å¾—ç›®å‰çš„ USD/TWD åŒ¯çŽ‡"""
...
@router.put("/exchange-rate", summary="è¨­å®šåŒ¯çŽ‡")
async def update_rate(
    data: ExchangeRateUpdate,
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_session),
):
    """æ‰‹å‹•è¨­å®š USD/TWD åŒ¯çŽ‡"""
...
@router.post("/transactions", summary="æ–°å¢žäº¤æ˜“ç´€éŒ„")
async def create_transaction(
    ...
    """
    æ–°å¢žä¸€ç­†è‚¡ç¥¨äº¤æ˜“ç´€éŒ„
    
    - **symbol**: è‚¡ç¥¨ä»£ç¢¼ï¼ˆå¦‚ AAPLã€2330ï¼‰
    - **market**: å¸‚å ´é¡žåž‹ï¼ˆtw=å°è‚¡, us=ç¾Žè‚¡ï¼‰
    ...
    """
    logger.info(f"æ–°å¢žäº¤æ˜“: user={user.id}, {data.transaction_type} {data.quantity} {data.symbol}")
    ...
    try:
        ...
        logger.error(f"æ–°å¢žäº¤æ˜“å¤±æ•—: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="æ–°å¢žäº¤æ˜“å¤±æ•—")




@router.get("/transactions/last-price/{symbol}", summary="å–å¾—è‚¡ç¥¨æœ€å¾Œäº¤æ˜“åƒ¹æ ¼")
async def get_last_transaction_price(
    ...
    """
    å–å¾—ç”¨æˆ¶å°è©²è‚¡ç¥¨çš„æœ€å¾Œä¸€ç­†äº¤æ˜“åƒ¹æ ¼
    ç”¨æ–¼æ–°å¢žäº¤æ˜“æ™‚è‡ªå‹•å¸¶å…¥é è¨­åƒ¹æ ¼
    """
    try:
        ...
        if transaction:
            ...
            return {
                "success": False,
                "message": "ç„¡æ­·å²äº¤æ˜“è¨˜éŒ„"
            }
    except Exception as e:
        logger.error(f"æŸ¥è©¢æœ€å¾Œäº¤æ˜“åƒ¹æ ¼å¤±æ•—: {e}")
        return {
            "success": False,
            "message": str(e)
        }
...
@router.get("/holdings", summary="å–å¾—æŒè‚¡ç¸½è¦½")
async def get_holdings(
    ...
    """
    å–å¾—ç”¨æˆ¶çš„æŒè‚¡åˆ—è¡¨ï¼ˆå«ç¾åƒ¹å’Œæç›Šï¼‰
    
    - ç¾åƒ¹ä¾†è‡ªåƒ¹æ ¼å¿«å–
    - åŒ…å«æœªå¯¦ç¾æç›Šè¨ˆç®—
    """
    try:
        service = PortfolioService(db)
        ...
        logger.error(f"å–å¾—æŒè‚¡å¤±æ•—: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="å–å¾—æŒè‚¡å¤±æ•—")
